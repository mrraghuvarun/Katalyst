...
import { FaArrowLeft, FaArrowRight, FaTachometerAlt, FaTable, FaLayerGroup, FaUndo, FaArrowAltCircleRight, FaSignOutAlt, FaFileUpload, } from "react-icons/fa";
import "./Sidebar.css";
const Sidebar = ({ collapsed, toggleSidebar }) => {
    ...
    return (<div className={`sidebar ${collapsed ? "collapsed" : ""}`}>
      ...
      <ul className="nav-links">
        <li>
          <a href="/summary" className={`nav-item ${["/summary", "/report", "/data"].includes(location.pathname)
            ? "active"
            : ""}`}>
        ...
        <li>
          <a href="/trade" className={`nav-item ${location.pathname === "/trade" ? "active" : ""}`}>
            <FaTable className="icon"/>
            <span className="nav-text">{!collapsed && "Trade Report"}</span>
          </a>
        </li>
        <li>
          <a href="/backreporting" className={`nav-item ${location.pathname === "/backreporting" ? "active" : ""}`}>
